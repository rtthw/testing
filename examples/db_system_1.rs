...
fn main() {
    ...
    db.put(rec_2, (Health(50.0), Regen(2.0), Armor(7.0)));
    db.put(rec_3, (Armor(3.0),));

    for (i, table) in db.tables.tables.iter().enumerate() {
        println!("TABLE #{i}: {:?}", table.type_ids);
    }

    {
        let r1 = db.record(rec_1);
        let r2 = db.record(rec_2);
        let r3 = db.record(rec_3);

        assert!(Column::<Health>::new(r1.table).is_some());
        assert!(Column::<Health>::new(r2.table).is_some());
        assert!(Column::<Health>::new(r3.table).is_none());

        assert!(Column::<Regen>::new(r1.table).is_some());
        assert!(Column::<Regen>::new(r2.table).is_some());
        assert!(Column::<Regen>::new(r3.table).is_none());

        assert!(Column::<Armor>::new(r1.table).is_none());
        assert!(Column::<Armor>::new(r2.table).is_some());
        assert!(Column::<Armor>::new(r3.table).is_some());
    }

    {
        ...
        assert!(*all_healths.get(rec_2.id as usize).unwrap() == Health(50.0));
        assert!(*all_regens.get(rec_2.id as usize).unwrap() == Regen(2.0));
    }
}
...
impl Database {
    pub fn new() -> Self {
        Self {
            ...
            tables: TableSet::new(),
            fields_to_table: HashMap::default(),
        }
    }
    ...
    pub fn put(&mut self, record: Record, fields: impl FieldSet) {
        let table_id = match fields.key() {
            Some(k) => {
                let tables = &mut self.tables;
                *self.fields_to_table.entry(k).or_insert_with(|| {
                    fields.with_ids(|ids| tables.get(ids, || fields.type_info()))
                })
            }
            None => fields.with_ids(|ids| self.tables.get(ids, || fields.type_info())),
        };
